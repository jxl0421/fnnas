...
2. **风扇自动控制**：根据CPU温度自动调节风扇转速
   ...
   - 3档位调节：0/150/200/255
   - 温度自动调节
3. **无线网卡配置**：内置WiFi模块，已配置中国大陆标准
   - 国家代码：CN（中国）
   - 监管域：CN
   - 2.4GHz信道：1-13（中国标准）
   - 5GHz信道：36-64, 149-165（中国标准）
   - 功率限制：20dBm（符合中国法规）
4. **按键自定义**：自定义电源键和复位键功能
5. **LED指示灯**：适配状态灯和磁盘活动灯
   - 状态LED：GPIOX_6（心跳模式）
   - 磁盘LED：GPIOX_17（mmc0活动）
...
- **启动参数调整**：优化启动参数为 `root=/dev/mmcblk0p1 rootfstype=ext4 rootwait console=ttyS0,115200n8 console=tty0`
- **启动地址设置**：启动地址调整为 `0x10800000`（T95标准启动地址）
- **内存配置**：1GB内存配置优化
- **DTB配置**：使用T95兼容的 `meson-gxl-s905x-p212.dtb`
- **编译方式**：使用HG680P的打包方式（s905x2-hg680fj）
- **硬件驱动**：用户空间GPIO/I2C控制，支持自动降级

## 注意事项

1. **DTB兼容性**：使用T95通用DTB（meson-gxl-s905x-p212.dtb），确保启动兼容性
2. **编译方式**：使用HG680P的编译方式（s905x2-hg680fj），确保与标准流程一致
3. **用户空间驱动**：OLED和风扇通过用户空间GPIO/I2C控制，不依赖内核驱动
4. **服务可靠性**：所有服务支持自动重试和优雅降级
5. **首次启动**：首次启动会自动运行install.sh配置系统

如遇到启动问题，请检查：
- 启动参数是否匹配
- DTB文件是否存在：`ls /boot/dtb/amlogic/meson-gxl-s905x-p212.dtb`
- 硬件服务状态：`systemctl status cumbox2-oled cumbox2-fan`
...
- **DTB问题**：确保使用CumeBox2现有的DTB（meson-gxl-s905x-cumebox.dtb）
